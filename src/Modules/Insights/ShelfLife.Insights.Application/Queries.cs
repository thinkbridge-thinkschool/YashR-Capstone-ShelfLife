...
{
    Task<PagedList<PopularTitleDto>> GetPopularTitlesAsync(int page, int pageSize, CancellationToken ct = default);
    Task<PagedList<OverdueLoanDto>> GetOverdueLoansAsync(int page, int pageSize, CancellationToken ct = default);
    Task<PagedList<MemberActivityDto>> GetMemberActivityAsync(int page, int pageSize, CancellationToken ct = default);
}

public sealed record GetPopularTitlesQuery(int Page = 1, int PageSize = 20);
public sealed record GetOverdueLoansQuery(int Page = 1, int PageSize = 20);
...
public sealed class InsightsQueryHandler
{
    ...
    public Task<PagedList<PopularTitleDto>> HandleAsync(GetPopularTitlesQuery q, CancellationToken ct = default) =>
        _readModel.GetPopularTitlesAsync(q.Page, q.PageSize, ct);

    public Task<PagedList<OverdueLoanDto>> HandleAsync(GetOverdueLoansQuery q, CancellationToken ct = default) =>
        _readModel.GetOverdueLoansAsync(q.Page, q.PageSize, ct);
